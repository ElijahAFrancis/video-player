...
//       // in development you must change `https://instaclip-5c26d78800f1.herokuapp.com/upload` to `http://${window.location.hostname}:3001/upload`
...
function Upload() {
  const [videoURL, setVideoURL] = useState(null);
  const [publicLink, setPublicLink] = useState(null); // Added state to store public link
  const [uploadVideo] = useMutation(UPLOAD_VIDEO);
  ...
  return (
    <div>
      ...
      {videoURL && <ReactPlayer url={videoURL} controls />}
      {publicLink && <p>Public Link: {publicLink}</p>} {/* Display the public link */}
    </div>
  );
}
